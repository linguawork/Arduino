...
void handleRoot() {
    ...
    for (int i = 1; i <= NUM_LEDS; i++) {
        html += "<button id=\"button" + String(i) + "\" onclick=\"sendRequest(" + String(i) + ")\">LED " + String(i) + "</button><br>";
    }
    html += "<button id=\"button" + String(NUM_LEDS + 1) + "\" onclick=\"sendPatternRequest()\">Pattern</button><br>";
    html += "</body></html>";
    server.send(200, "text/html", html);
}


//modified in the 4th version
...
//version with Pattern
void showPattern() {
    // Implement your pattern behavior here
    // For example, you could alternate between two colors
...
void handlePattern() {
    showPattern(); // Call the pattern function
    server.send(200, "text/plain", "Pattern activated");
}
...
void setup() {
  ...
  server.on("/pattern", HTTP_GET, handlePattern); // Handle the pattern
  server.begin();
